...
export default class CreateLocations implements Seeder {
  public async run(factory: Factory, connection: Connection): Promise<any> {
    ...
    const cityCount = await connection
    ...
    const wardCount = await connection
      .getRepository(WardEntity)
      .count();
    if (wardCount <= 0) {
      await connection
        .createQueryBuilder()
